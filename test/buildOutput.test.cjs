...
test('build output includes the realtime audio worklet module', () => {
  const workletPath = path.join(__dirname, '..', 'dist', 'renderer', 'voice', 'audioFrameProcessor.js');
  assert.ok(
    fs.existsSync(workletPath),
    'build should copy the audio worklet processor into dist/renderer/voice',
  );
});

test('build output includes the renderer bootstrap script', () => {
  const bootstrapPath = path.join(__dirname, '..', 'dist', 'renderer', 'bootstrap.js');
  assert.ok(
